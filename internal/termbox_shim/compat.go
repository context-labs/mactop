...
func SetOutputMode(mode OutputMode) OutputMode {
	// Force 256 color mode to prevent termui from downgrading us to 16 colors (OutputNormal).
	// This ensures attributes > 15 (like 245) are not masked to 0 (Black).
	outMode = Output256
	return outMode
}
...
func makeEvent(tev tcell.Event) Event {
	...
	case *tcell.EventMouse:
		...
		// Map Tcell buttons to Termbox keys
		if b&tcell.WheelUp != 0 {
			k = MouseWheelUp
		} else if b&tcell.WheelDown != 0 {
			k = MouseWheelDown
		} else if b&tcell.Button1 != 0 {
			k = MouseLeft
		} else if b&tcell.Button2 != 0 {
			k = MouseMiddle
		} else if b&tcell.Button3 != 0 {
			k = MouseRight
		} else if b == tcell.ButtonNone {
			// Check for release event if needed, but for now ignoring moves/releases
			// as termui mainly cares about clicks/scrolls for lists.
